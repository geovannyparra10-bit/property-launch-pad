...
import { generateLeaseHTML } from './leaseAgreementText'
import { defaultLeaseData } from './leaseTypes'
...
const AGREEMENT_STYLES = `
  ...
  @media print {
    ...
    #lease-print-area, #lease-print-area * { visibility: visible; }
    #lease-print-area {
...
export function LeaseAgreement() {
  ...
  const agreementHTML = generateLeaseHTML(formData, language)

  const handleDownload = () => {
    if (!isPremium) { setShowPremiumModal(true); return }

    const printArea = document.getElementById('lease-print-area')
    if (!printArea) return
  ...
  const L = language === 'en' ? {
    ...
    subtitle: 'Fill in the form to generate your lease agreement',
    back: 'Back to Templates',
    form: 'Lease Details',
    preview: 'Live Preview',
    download: 'Download Lease PDF',
    ...
    subtitle: 'Complete el formulario para generar su contrato de arrendamiento',
    back: 'Volver a Plantillas',
    form: 'Detalles del Contrato',
    preview: 'Vista Previa en Vivo',
    download: 'Descargar Contrato en PDF',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid xl:grid-cols-2 gap-6">
          <div>
            ...
            <LeaseForm data={formData} onChange={handleChange} lang={language} />
          </div>
